  ...
  return (
    <div className="relative min-w-0 flex-1 sm:max-w-md">
      <input
        ...
        className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-slate-400 focus:bg-white"
      />
      {trimmed !== "" && (
            ...
            <ul className="max-h-72 overflow-auto py-1">
              {visible.map((node) => (
                <li key={node.id}>
                  ...
                  >
                    <span className={node.type === "folder" ? "text-sky-600" : "text-slate-400"}>
