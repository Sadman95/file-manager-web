...
- **Editor draft is local:** typing never touches global state; only Save dispatches, so navigation + refresh semantics are explicit (confirm / `beforeunload`). The open file's name is inline-editable in the editor header (click to edit, blur to finish, Escape to revert) and saves together with content only when either changed. `Cmd/Ctrl+S` saves, Tab indents, and browsing to another folder closes the editor (after confirming dirty edits).
- **File icons are free MIT glyphs, vendored offline:** per-extension `filetype-*` + generic `file-earmark-*` SVGs from Bootstrap Icons (`npm i bootstrap-icons`, codegen via `npm run generate:icons`, unknown exts fall back to a generic glyph). No icon API/CDN — offline, zero runtime deps, and realtime everywhere because the icon is a pure function of the name string.
- **One open thing at a time:** selecting a folder closes the editor; opening a file lands the panel on its parent. New files open immediately after creation. `react-hot-toast` confirms create/rename/delete/save; dialogs trap Tab focus; cards show recency/counts and stay actionable on touch.
- **Root is undeletable**, deleting a selected folder reselects the nearest surviving parent, and deleting an open file closes it — all covered by reducer unit tests (`20` tests across domain, reducer, schema, and time).
